...
void WebServer::addConnection(int client_fd) {
	...
	_lggr.debug("Added connection tracking for fd: " + string_utils::to_string(client_fd));
}
...
void WebServer::updateConnectionActivity(int client_fd) {
	...
	if (it != _connections.end()) {
		...
		_lggr.debug("Updated activity for fd: " + string_utils::to_string(client_fd));
	}
}
...
void WebServer::cleanupExpiredConnections() {
	...
	for (std::map<int, ConnectionInfo *>::iterator it = _connections.begin();
		...
		if (isConnectionExpired(conn)) {
			...
			_lggr.info("Connection expired for fd: " + string_utils::to_string(conn->clfd));
		}
	}
	...
	if (!_conn_to_close.empty()) {
		_lggr.info("Cleaned up " + string_utils::to_string(_conn_to_close.size()) +
		           " expired connections");
	}
}
...
void WebServer::handleConnectionTimeout(int client_fd) {
	...
	if (it != _connections.end()) {
		...
		_lggr.info("Connection timed out for fd: " + string_utils::to_string(client_fd) +
		           " (idle for " + string_utils::to_string(getCurrentTime() - conn->last_activity) +
		           " seconds)");
	}
...
bool WebServer::shouldKeepAlive(const ClientRequest &req) {
	...
	if (conn->request_count >= MAX_KEEP_ALIVE_REQS) {
		_lggr.debug("Max keep-alive requests reached for fd: " + string_utils::to_string(req.clfd));
		return false;
	}
...
void WebServer::closeConnection(int client_fd) {
	_lggr.debug("Closing connection for fd: " + string_utils::to_string(client_fd));
	...
	_lggr.debug("Connection cleanup completed for fd: " + string_utils::to_string(client_fd));
}
